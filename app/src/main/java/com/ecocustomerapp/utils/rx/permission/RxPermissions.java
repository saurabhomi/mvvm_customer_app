package com.ecocustomerapp.utils.rx.permission;
...
public class RxPermissions {
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public Observable<Boolean> shouldShowRequestPermissionRationale(final Activity activity, int code, final String... permissions) {
        if (!isMarshmallow()) {
            return Observable.just(false);
        }
        String[] strings;
        if (permissions[0].contains(",")) {
            strings = permissions[0].split(",");
        } else {
            strings = permissions;
        }


        return Observable.just(shouldShowRequestPermissionRationaleImplementation(activity, code, strings));
    }

    @TargetApi(Build.VERSION_CODES.M)
    private boolean shouldShowRequestPermissionRationaleImplementation(final Activity activity, int code, final String... permissions) {
        for (String p : permissions) {
